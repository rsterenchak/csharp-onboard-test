...
This file documents the project-specific commands, conventions, and quirks for **csharp-onboard-test**. It supplements `routine-base.md` (which has the universal discipline) with project-specific details Claude runs need to know.
...
- **Source code:** ``
- **Tests:** `tests/`
- **Build output:** `bin/` (typically `dist/` or `build/`)
...
**Deploy target:** none (console app)
...
**Variant:** (none — console project) (either `gen-src-manifest.js` for CommonJS projects or `gen-src-manifest.cjs` for ESM projects with `"type": "module"` in package.json)
...
>
> - "csharp-onboard-test is a single-page app; routing is hash-based via {{ROUTER_FILE}}."
...
>
> - "Run `dotnet test` before opening the PR — CI will catch failures but the agent should not rely on CI to find them."
