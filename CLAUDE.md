...
**Files to fill in:**
- [ ] Replace `csharp-onboard-test` everywhere in this file with this project's name (the repo name is usually fine: e.g. `my-app`, `react-thing`, `cool-project`).
...
- [ ] Replace `dotnet test` with the command CI runs to test (e.g. `npm test`, `npm run test:ci`, `vitest run`).
- [ ] Replace `dotnet build` with the command CI runs to build (e.g. `npm run build`, `vite build`).
- [ ] Replace `none (console app)` with where the project deploys (e.g. `GitHub Pages`, `Cloudflare Pages`, `Vercel`, or `none — library, no deploy`).
...
- [ ] `.github/workflows/test.yml` exists and runs `dotnet test`.
- [ ] `.github/workflows/deploy.yml` exists, runs `dotnet build`, and includes a step to run `scripts/gen-src-manifest.{js|cjs}` so the in-app Claude assistant can attach files from this repo.
...
**Project name:** csharp-onboard-test
